<template>
  <div>
    <v-card class="overline">
      ...
      <v-card-subtitle v-else-if="matches.length === 1">Displaying 1 match.</v-card-subtitle>
    ...
    <v-card class="overline my-5" v-for="(match, i) in matches" :key="i" cols="12">
      ...
      <v-card-subtitle>Participants: {{ match.participants.length }}<br>Budget: {{ match.budget }}</v-card-subtitle>
      <v-card-actions>
